...
def ADefinir2(cadena):
    ...
    for token in Tokens():
        for encontrado in re.finditer(token, cadena): # busca los tokens que hay en la cadena
            ...
            if encontrado.group() == 'como': # cuando encuentra dónde dice como guarda su indice
                iniciocomo = encontrado.start()-1
                #print (iniciocomo)
            #print("******************************************")

            variablescadena = cadena[8: int(iniciocomo)].strip()

    definir = cadena[0:7] # corto los primeros 8 caracteres
    finalcadena = cadena[iniciocomo + 1:len(cadena)].replace(' ', '')# corto la ultima parte de la cadena

    if definir != "definir" or (finalcadena != "comoentero;" and finalcadena != "comocaracter;" and finalcadena != "comologico;" and finalcadena != "comoreal;"):
        return False
